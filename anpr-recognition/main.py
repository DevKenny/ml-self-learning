...
for imagePath in imagePaths:
    ...
    (lp, lpCnt) = anpr.find_and_ocr(image, psm=args['psm'], clearBorder=args['clear_border'] > 0)

    if lp is not None and lpCnt is not None:
        box = cv2.boxPoints(cv2.minAreaRect(lpCnt))
        box = box.astype('int')
        cv2.drawContours(image, [box], -1,  (0, 255, 0), 2)

        (x, y, h, w) = cv2.boundingRect(lpCnt)
        cv2.putText(image, cleanup_text(lp), (x, y - 15),
